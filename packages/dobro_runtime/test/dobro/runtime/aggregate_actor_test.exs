defmodule Dobro.Runtime.AggregateActorTest do
  ...
  describe "actor_name/3" do
    test "global scope uses a global tenant partition" do
      assert AggregateActor.actor_name(SampleAggregate, nil, %{id: 1}) ==
               :"aggregate_actor_Elixir.Dobro.Runtime.AggregateActorTest.SampleAggregate_global_id:1"
    end

    test "keys by tenant id so different tenants get distinct actors" do
      name_60 =
        AggregateActor.actor_name(
          SampleAggregate,
    ...
    test "identifier renames do not change the actor name" do
      identity = %{id: 2}

      assert AggregateActor.actor_name(
               SampleAggregate,
    ...
    test "identity map key order does not affect the actor name" do
      tenant = TenantContext.new(id: 1, identifier: "acme")

      assert AggregateActor.actor_name(SampleAggregate, tenant, %{id: 2, office_id: 9}) ==
               AggregateActor.actor_name(SampleAggregate, tenant, %{office_id: 9, id: 2})
    end
